...
def parse_opts_dmd2():
    ...
    """ DMD2 Setting """
    parser.add_argument(
        "--gen_network", type=str, default="sngan", choices=["sagan", "sngan"]
    )
    ...
    parser.add_argument("--train_amp", action="store_true", default=False)
    parser.add_argument(
        "--adv_loss_type", type=str, default="hinge", choices=["vanilla", "hinge"]
    )
    parser.add_argument("--train_batch_size", type=int, default=16, metavar="N")
    parser.add_argument("--gradient_accumulate_every", type=int, default=1, metavar="N")
    parser.add_argument(
        "--train_lr_generator", type=float, default=1e-4, help="learning rate"
    )
    parser.add_argument(
        "--train_lr_guidance", type=float, default=1e-4, help="learning rate"
    )  # lr for both discriminator branch and fake unet
    parser.add_argument(
        "--num_D_steps",
        type=int,
        default=4,
    ...
    parser.add_argument(
        "--weight_guidance_adv", type=float, default=1e-3
    )  # loss weight for the fake_unet+netD
    parser.add_argument(
        "--weight_generator_adv", type=float, default=1e-3
    )  # loss weight for netG
    ...
    parser.add_argument(
        "--gan_DiffAugment_policy", type=str, default="color,translation,cutout"
    )
